...
export class OccasionComponent implements OnInit {
  ...
  filteredArray: any[] = [];
  paginatedArray: any[] = [];
  modalForm: FormGroup;  // Form group for managing occasion form
  ...
  fetchAll() {
    this.actionsService.occasions$.subscribe((entities) => {
      this.fetchedArray = entities;
      console.log(entities)
      this.filter();  // Initially filter the occasions
    });
  }

  filter() {
    if (this.searchText) {
      // Filter occasions based on the search text
      this.filteredArray = this.fetchedArray.filter((ent) =>
        ent.title.toLowerCase().includes(this.searchText.toLowerCase())
      );
    } else {
      // If no search text, show all occasions
    ...
    try {
      ...
      onActionSelected(selectedAction: any) {
        if (selectedAction && selectedAction.length > 0) {
          ...
          if (action.title === 'Add') {
            this.open(this.modalContent);  // Open modal to add a occasion
          } 
          else if (action.title === 'Seed') {
            this.seed();  // Call seed occasions
          }
          else if (action.title === 'Delete All') {
            this.deleteAll();  // Call delete all occasions
          }
        }
      }
  ...
  async saveModal(): Promise<void> {
    try {
      ...
      if (this.isUpdateMode) {
        await this.actionsService.updateOccasion(data);  // Update occasion
      } else {
        await this.actionsService.createOccasion(data);  // Create new occasion
      }

      this.modalService.dismissAll();  // Close modal
      this.actionsService.refreshOccasions();
    } catch (error) {
  ...
  getData(): Occasion {
    ...
    const data: Occasion = new Occasion(this.isUpdateMode ? Operations.Update : Operations.Create);
  ...
  async delete(id: number): Promise<void> {
    if (confirm('Are you sure you want to delete this Occasion?')) {
      try {
        await this.actionsService.deleteOccasion(id);
        this.actionsService.refreshOccasions();
      } catch (error) {
  ...
  async deleteAll(): Promise<void> {
    if (confirm('Are you sure you want to delete all occasions? This action cannot be undone.')) {
      try {
        await this.actionsService.deleteAllOccasions();
        this.actionsService.refreshOccasions();
      } catch (error) {
        console.error('Error deleting all occasions', error);
      }
    }
  }
  async seed(): Promise<void> {
    if (confirm('Are you sure you want to seed new occasions?')) {
      try {
        await this.actionsService.seedOccasions();
        this.actionsService.refreshOccasions();
      } catch (error) {
