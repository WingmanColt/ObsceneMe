...
export class RedZoomDirective implements AfterContentInit, OnChanges, OnDestroy {
    ...
    @Input() productImage: string = ''; // first fallback
    @Input() lazyLoad: string = ''; // Second fallback
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if ('productImage' in changes && !changes['productImage'].firstChange) {
            this.onChangeThumbSrc();
        }
        if ('lazyLoad' in changes && !changes['lazyLoad'].firstChange) {
            this.onChangeThumbSrc();
        }
...
getThumbSrc(): string {
    if (!this.isImage || !this.src) {
        if (this.productImage) {
            return this.productImage; // First fallback
        }

        if (this.lazyLoad) {
            return this.lazyLoad; // Second fallback
        }

        return this.thumbSrc; // Default fallback
    }

    return this.src; // Primary source
}
    ...
    mouseEnter = (event: MouseEvent): void => {
        ...
        this.session = {
            active: false,
            thumbSize: { x: 50, y: 50 }, // Use x and y instead of width and height
            thumbPos: { x: 0, y: 0 }, // Use x and y instead of top and left
            lensContainerSize: { x: 200, y: 200 }, // Use x and y for size
            lensImageSize: { x: 100, y: 100 }, // Use x and y for size
            frameSize: { x: 100, y: 100 }, // Use x and y for size
        ...
        const onWheel = (wheelEvent: WheelEvent) => {
            ...
            // Update the scale factor with delta and constrain it within the range [1, 10]
            this.scaleFactor += 0.3 * -delta;
    ...
    move(): void {
        ...
        if (!frameSize || !thumbPos || !thumbSize || !lensImageSize || !lensContainerSize) {
            console.log(frameSize, thumbPos, thumbSize, lensImageSize, lensContainerSize)
